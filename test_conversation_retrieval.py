...
def test_chat_get_route():
    ...
    conversation_id = conversation_manager.create_conversation(test_username, "GET Route Test")
    conversation_manager.add_message(test_username, conversation_id, "user", "Hello from GET test")
    conversation_manager.add_message(test_username, conversation_id, "assistant", "Hi there! This is a test response.", "resp_get_001")
    conversation_manager.add_message(test_username, conversation_id, "user", "How are you?")
    conversation_manager.add_message(test_username, conversation_id, "assistant", "I'm doing well, thank you!", "resp_get_002")
    ...
    with app.test_client() as client:
        ...
        assert response_data["threadId"] == conversation_id, f"Expected {conversation_id}, got {response_data['threadId']}"
...
def test_get_all_conversations_route():
    ...
    conversations_data = [
        {"name": "First Conversation", "messages": [("user", "Hello 1"), ("assistant", "Hi 1", "resp_list_001")]},
        {"name": "Second Conversation", "messages": [("user", "Hello 2"), ("assistant", "Hi 2", "resp_list_002")]},
        {"name": "Third Conversation", "messages": [("user", "Hello 3")]}  # No assistant response yet
    ]
    
    created_conversations = []
    
    for conv_data in conversations_data:
        conv_id = conversation_manager.create_conversation(test_username, conv_data["name"])
        created_conversations.append(conv_id)
        
        for message in conv_data["messages"]:
            role, text = message[:2]
            response_id = message[2] if len(message) > 2 else None
            conversation_manager.add_message(test_username, conv_id, role, text, response_id)
    ...
    with app.test_client() as client:
        ...
        for conv_id, conv_data in conversations.items():
            assert conv_id in created_conversations, f"Unexpected conversation ID: {conv_id}"
            
            # Check required fields
            required_fields = ["data", "chat_name", "last_update"]
            for field in required_fields:
                assert field in conv_data, f"Conversation {conv_id} missing field: {field}"
            
            # Check data structure
            data = conv_data["data"]
            assert "id" in data, f"Conversation {conv_id} data missing id"
            assert "created_at" in data, f"Conversation {conv_id} data missing created_at"
            assert "object" in data, f"Conversation {conv_id} data missing object"
            assert data["object"] == "conversation", f"Expected object=conversation, got {data['object']}"
            
            # Verify chat name matches what we created
            expected_names = [cd["name"] for cd in conversations_data]
            assert conv_data["chat_name"] in expected_names, f"Unexpected chat name: {conv_data['chat_name']}"
...
def test_conversation_manager_message_list():
    ...
    conversation_id = conversation_manager.create_conversation(test_username, "Message List Test")
    ...
    for message in test_messages:
        role, text = message[:2]
        response_id = message[2] if len(message) > 2 else None
        conversation_manager.add_message(test_username, conversation_id, role, text, response_id)
    ...
    assert len(message_list) == len(test_messages), f"Expected {len(test_messages)} messages, got {len(message_list)}"
    ...
    for i, (expected_role, expected_text, *_) in enumerate(test_messages):
        ...
        # Check content
        assert actual_message["role"] == expected_role, f"Message {i}: expected role {expected_role}, got {actual_message['role']}"
        assert actual_message["text"] == expected_text, f"Message {i}: expected text {expected_text}, got {actual_message['text']}"
...
def test_frontend_compatibility():
    ...
    conversation_id = conversation_manager.create_conversation(test_username, "Frontend Compatibility Test")
    
    # Add realistic conversation
    realistic_messages = [
        ("user", "Hello, I need help with Python programming."),
        ("assistant", "I'd be happy to help you with Python! What specific topic or problem are you working on?", "resp_fe_001"),
        ("user", "I'm trying to understand list comprehensions."),
        ("assistant", "List comprehensions are a concise way to create lists in Python. Here's a basic example:\n\n```python\n# Traditional way\nresult = []\nfor i in range(10):\n    if i % 2 == 0:\n        result.append(i * 2)\n\n# List comprehension\nresult = [i * 2 for i in range(10) if i % 2 == 0]\n```\n\nBoth create the same list: [0, 4, 8, 12, 16]", "resp_fe_002"),
    ...
    with app.test_client() as client:
        ...
        # Verify frontend-expected structure
        assert isinstance(conversations_data, dict), "Conversations response should be a dictionary"
        
        for conv_id, conv_data in conversations_data.items():
            assert isinstance(conv_id, str), "Conversation ID should be a string"
            assert isinstance(conv_data, dict), "Conversation data should be a dictionary"
            ...
            # Check data structure
            assert isinstance(conv_data["data"], dict), "Conversation data.data should be a dictionary"
            assert isinstance(conv_data["chat_name"], str), "chat_name should be a string"
            assert isinstance(conv_data["last_update"], (int, float)), "last_update should be a number"
...
def test_error_handling():
    ...
    with app.test_client() as client:
        ...
        response_data = json.loads(response.data.decode())
        assert response_data["messages"] == [], "Non-existent conversation should return empty messages"
    ...
    with app.test_client() as client:
        ...
        conversations_data = json.loads(response.data.decode())
        assert conversations_data == {}, "User with no conversations should return empty dict"
