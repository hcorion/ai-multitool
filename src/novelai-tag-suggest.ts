...
function isNovelAIActive(): boolean {
...
function getCurrentToken(textarea: HTMLTextAreaElement): { token: string; start: number } {
...
function buildDropdown(suggestions: TagSuggestion[], textarea: HTMLTextAreaElement): void {
    removeDropdown();
...
function setSelectedIndex(index: number): void {
    if (!dropdown) return;
    const items = dropdown.querySelectorAll<HTMLElement>(".nai-tag-suggestion-item");
    items.forEach((el, i) => el.classList.toggle("selected", i === index));
    selectedIndex = index;
}

function insertTag(tag: string, textarea: HTMLTextAreaElement): void {
    const cursor = textarea.selectionStart ?? 0;
...
async function fetchSuggestions(token: string): Promise<TagSuggestion[]> {
...
function onTextareaInput(e: Event): void {
    ...
    debounceTimer = setTimeout(async () => {
        ...
        if (activeTextarea !== textarea) return; // focus moved away
...
function attachToTextarea(textarea: HTMLTextAreaElement): void {
    ...
    textarea.addEventListener("blur", () => {
        ...
        setTimeout(() => {
            if (activeTextarea === textarea) removeDropdown();
        }, 150);
    });
}

/** Attach to a dynamically added character prompt textarea. */
export function attachTagSuggestToTextarea(textarea: HTMLTextAreaElement): void {
    attachToTextarea(textarea);
}

/** Initialize tag suggestions on the static prompt textareas. */
export function initTagSuggestions(): void {
    const prompt = document.getElementById("prompt") as HTMLTextAreaElement | null;
    const negPrompt = document.getElementById("negative_prompt") as HTMLTextAreaElement | null;

    if (prompt) attachToTextarea(prompt);
    if (negPrompt) attachToTextarea(negPrompt);
