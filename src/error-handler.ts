...
export interface ErrorDisplayOptions {
    /** Element to display the error in (defaults to console) */
    targetElement?: HTMLElement | null;
    /** Whether to show technical details */
    showDetails?: boolean;
    /** Custom CSS class for error styling */
    errorClass?: string;
    /** Duration to show error before auto-hiding (0 = no auto-hide) */
...
export interface RetryOptions {
    /** Maximum number of retries after initial attempt (total attempts = maxRetries + 1) */
    maxRetries?: number;
...
export function isErrorResponse(value: unknown): value is ErrorResponse {
    return (
        ...
        value.success === false &&
        'error_type' in value &&
        typeof (value as ErrorResponse).error_type === 'string' &&
        'error_message' in value &&
        typeof (value as ErrorResponse).error_message === 'string' &&
        'timestamp' in value &&
        typeof (value as ErrorResponse).timestamp === 'number'
    );
}
...
export function isRetryableError(error: unknown): boolean {
    ...
    if (error && typeof error === 'object') {
        const errorType = 
            ('error_type' in error && typeof error.error_type === 'string' ? error.error_type : '') ||
...
let errorDisplayTimeout: number | null = null;

/**
 * Clear error display
 */
export function clearError(targetElement: HTMLElement): void {
...
export async function handleApiResponse<T>(response: Response): Promise<T> {
    if (!response.ok) {
        ...
        if (errorData && typeof errorData === 'object') {
            (errorData as Record<string, unknown>).status = response.status;
        }
 ...
 */
export function parseJQueryError(xhr: JQueryXHR): ErrorResponse {
    ...
    if (xhr.responseJSON) {
        ...
        if (json && typeof json === 'object') {
            ...
            if ('error_details' in json && typeof json.error_details === 'object') {
                errorDetails = json.error_details as Record<string, unknown>;
            }
        }
