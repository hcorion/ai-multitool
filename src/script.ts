...
function updateInpaintingStrengthNoiseLabels() {
    const strengthSlider = document.getElementById("inpainting-strength") as HTMLInputElement | null;
    const noiseSlider = document.getElementById("inpainting-noise") as HTMLInputElement | null;
...
function syncInpaintingNovelaiStrengthNoiseEnabled() {
    ...
    const strengthEl = document.getElementById("inpainting-strength") as HTMLInputElement | null;
    const noiseEl = document.getElementById("inpainting-noise") as HTMLInputElement | null;
...
function providerChanged() {
    ...
    if (selection.value == "openai") {
        ...
        const sizeSelect = getElementByIdSafe("size", HTMLSelectElement);
        if (sizeSelect) sizeSelect.selectedIndex = 3;
        modelChanged(selection.value);
    } else {
...
function updateGridModalImage(): void {
    ...
    (document.getElementById("grid-modal-image") as HTMLImageElement).src = imageInfo.fullImagePath;
 ...
 */
function handleGridImageNavigation(): { fullImagePath: string; fileName: string } | null {
 ...
 */
function processImageMetadata(metadata: any): {
    processedMetadata: { [key: string]: string };
    characterPromptData: { [key: string]: { base: string; processed: string; isDifferent: boolean } };
    mainPromptData: { [key: string]: { base: string; processed: string; isDifferent: boolean } };
} {
 ...
 */
function processCharacterPrompt(
    metadata: any,
    characterMatch: RegExpMatchArray,
    processedMetadata: { [key: string]: string },
    characterPromptData: { [key: string]: { base: string; processed: string; isDifferent: boolean } }
): void {
 ...
 */
function processMainPrompt(
    metadata: any,
    mainPromptMatch: RegExpMatchArray,
    processedMetadata: { [key: string]: string },
    mainPromptData: { [key: string]: { base: string; processed: string; isDifferent: boolean } }
): void {
 ...
 */
function attachMetadataActions(metadataDiv: HTMLElement, metadata: any): void {
    ...
    let generateVibeButton = document.getElementById("generate-vibe-btn") as HTMLButtonElement;
 ...
 */
function showVibeConfirmationDialog(imageFileName: string): void {
    ...
    const nameInput = document.getElementById('vibe-name-input') as HTMLInputElement;
    nameInput?.focus();
 ...
 */
async function startVibeGeneration(imageFileName: string, vibeName: string): Promise<void> {
    try {
 ...
 */
function showVibeSuccess(message: string): void {
 ...
 */
function showVibeError(message: string): void {
 ...
 */
async function createBlankCanvas(): Promise<void> {
...
async function openInpaintingMaskCanvas(imageUrl: string, originalPrompt?: string | undefined, originalNegativePrompt?: string | undefined, originalCharacterPrompts?: CharacterPromptData[]): Promise<void> {
    const canvas = new InpaintingMaskCanvas({
        ...
        onMaskComplete: async (maskDataUrl: string, maskFileId?: string | null, paintedImageDataUrl?: string) => {
            try {
 ...
 */
async function setupInpaintingMode(baseImageUrl: string, maskDataUrl: string, maskFileId?: string | null, originalPrompt?: string, originalNegativePrompt?: string, originalCharacterPrompts?: CharacterPromptData[], paintedImageDataUrl?: string): Promise<void> {
    try {
        // If we have a painted image, save it and use it as the base image
        let effectiveBaseImageUrl = baseImageUrl;
        let effectiveBaseImagePath: string;
        let maskPath = '';
        ...
        if (hasPainted && hasMask) operation = 'combined';
        else if (hasPainted && !hasMask) operation = 'img2img';
        else operation = 'inpaint';
 ...
 */
async function savePaintedImageToServer(paintedImageDataUrl: string): Promise<string> {
    const formData = new FormData();
    ...
    if (!saveResponse.ok) throw new Error('Failed to save painted image');
    const result = await saveResponse.json();
 ...
 */
function showInpaintingSection(baseImageUrl: string, maskDataUrl: string, baseImagePath: string, maskPath: string, operation: string = 'inpaint'): void {
...
function sendChatMessage(): void {
    ...
    fetchWithStreaming(
        ...
        (chunkData) => {
            ...
            const chatData: chat.MessageHistory = typeof parsedData === "string" 
...
function processChatChunk(
    chatData: chat.MessageHistory,
    chatName: string,
    chatInput: HTMLTextAreaElement,
    sendChatButton: HTMLInputElement
): void {
 ...
 */
function handleMessageListChunk(
    chatData: chat.MessageHistory,
    chatName: string,
    chatInput: HTMLTextAreaElement,
    chatStatusText: HTMLDivElement
): void {
 ...
 */
function handleTextCreatedChunk(chatStatusText: HTMLDivElement): void {
 ...
 */
function handleTextDeltaChunk(chatData: chat.MessageHistory, chatStatusText: HTMLDivElement): void {
 ...
 */
function handleTextDoneChunk(sendChatButton: HTMLInputElement, chatStatusText: HTMLDivElement): void {
 ...
 */
function getReasoningHttpErrorMessage(status: number, statusText: string): string {
    switch (status) {
        case 404:
 ...
 */
function getReasoningFetchErrorMessage(error: any): string {
