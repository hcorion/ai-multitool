...
class TestEmphasisRangeProcessing:
    ...
    def test_range_emphasis_generates_decimal(self, temp_dir):
        ...
        assert len(str(generated_value).split(".")[1]) <= 2  # At most 2 decimal places

    def test_decimal_precision_two_places(self, temp_dir):
        """Test that decimal values are rounded to at most 2 decimal places"""
        # Requirement 1.4: Values should be rounded to at most 2 decimal places
        prompt = "1.0-2.0::test::"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=123)

        # Extract the generated value
        match = re.search(r"(\d+\.\d{1,2})::test::", result)
        assert match is not None

        value_str = match.group(1)
        # Should have 1 or 2 digits after decimal point
        assert re.match(r"\d+\.\d{1,2}$", value_str)
    ...
    def test_zero_values(self, temp_dir):
        ...
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == "0.0::zero emphasis::"
    ...
    def test_multiple_emphasis_ranges_in_prompt(self, temp_dir):
        ...
        first_match = re.search(r"(\d+\.\d{1,2})::first::", result)
        second_match = re.search(r"(\d+\.\d{1,2})::second::", result)
        ...
        assert abs(second_value - 3.0) < 0.001  # Allow for floating point precision
    ...
    def test_emphasis_with_special_characters(self, temp_dir):
        ...
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == "1.5::text with spaces and symbols!@#::"

    def test_decimal_formatting_precision(self, temp_dir):
        """Test that decimal formatting only shows necessary precision"""
        # Test cases from user request
        test_cases = [
            ("2.0::prompt::", "2.0::prompt::"),  # Should stay as 2.0, not become 2.00
            ("2::prompt::", "2.0::prompt::"),  # Should get corrected to 2.0
            ("2.353::prompt::", None),  # Should get rounded to 2.35 (tested with range)
        ]

        for input_prompt, expected in test_cases:
            if expected:
                result = make_prompt_dynamic(
                    input_prompt, "testuser", temp_dir, seed=42
                )
                assert result == expected, (
                    f"Input: {input_prompt}, Expected: {expected}, Got: {result}"
                )

        # Test 2.353 case with range to trigger rounding
        prompt = "2.353-2.353::prompt::"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == "2.35::prompt::", (
            f"Expected 2.353 to round to 2.35, got: {result}"
        )

    def test_whole_number_formatting(self, temp_dir):
        """Test that whole numbers get .0 added"""
        prompt = "2::content::"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == "2.0::content::"

        prompt = "10::content::"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == "10.0::content::"
...
class TestErrorHandlingAndInvalidSyntax:
    ...
    def test_choice_all_empty_options(self, temp_dir):
        ...
        prompt = "{||}"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        # With the updated regex, {||} now matches and returns empty string
        assert result == ""
    ...
    def test_choice_with_actual_empty_options(self, temp_dir):
        ...
        assert result in [
            ...
            "",
            "b",
        ]  # Should select one of these, including empty string
    
    def test_choice_with_empty_first_option(self, temp_dir):
        """Test choice with empty first option"""
        prompt = "{|thing}"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result in ["", "thing"]
    
    def test_choice_with_empty_last_option(self, temp_dir):
        """Test choice with empty last option"""
        prompt = "{, thing|}"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result in [", thing", ""]
    
    def test_choice_with_only_empty_options(self, temp_dir):
        """Test choice with only empty options"""
        prompt = "{|}"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == ""  # Should always return empty string
    
    def test_choice_multiple_empty_options(self, temp_dir):
        """Test choice with multiple empty options"""
        prompt = "{hello|world|}"
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
...
class TestEmptyAndMalformedInput:
    ...
    def test_very_long_content(self, temp_dir):
        ...
        result = make_prompt_dynamic(prompt, "testuser", temp_dir, seed=42)
        assert result == f"1.5::{long_content}::"
...
class TestIntegrationWithExistingFeatures:
    ...
    def test_character_prompts_with_new_syntax(self, temp_dir):
        ...
        character_prompts = [
            ...
            {"positive": "2.0::handsome:: person", "negative": ""},
        ]
        ...
        assert "2.0::handsome:: person" == second_positive
    ...
    def test_grid_prompt_override_with_new_syntax(self, temp_dir):
        ...
        result = make_prompt_dynamic(
            prompt, "testuser", temp_dir, seed=42, grid_prompt=grid_prompt
        )
