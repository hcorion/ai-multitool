 ...
 */
export async function initializeAgentPresetUI() {
    try {
 ...
 */
function renderAgentPresetSelector() {
    const selector = document.getElementById('agent-preset-selector');
    if (!selector)
        return;
 ...
 */
function renderReasoningLevelSelector() {
    const selector = document.getElementById('reasoning-level-selector');
    if (!selector)
        return;
    // Clear existing options
    selector.innerHTML = '';
...
async function handlePresetSelection(event) {
    ...
    try {
        ...
        agentPresets.handleAgentPresetError(error, 'select');
 ...
 */
function handleReasoningLevelSelection(event) {
    const selector = event.target;
    ...
    try {
        ...
        handleReasoningLevelError(error, 'set');
 ...
 */
function clearReasoningOverride() {
    try {
        ...
        handleReasoningLevelError(error, 'clear');
    }
}
/**
 * Update the reasoning level indicator in the input area
 */
function updateReasoningLevelIndicator() {
    try {
        ...
        handleReasoningLevelError(error, 'display');
    }
}
/**
 ...
 */
export function handleReasoningLevelError(error, operation, retryCount = 0) {
    ...
    if (retryCount < maxRetries) {
        ...
        setTimeout(() => {
            try {
                ...
                handleReasoningLevelError(retryError, operation, retryCount + 1);
            }
 ...
 */
function showReasoningLevelError(message) {
 ...
 */
function openAgentPresetModal(preset) {
    const modal = document.getElementById('agent-preset-modal');
    if (!modal)
        return;
    isModalOpen = true;
 ...
 */
function closeAgentPresetModal() {
    const modal = document.getElementById('agent-preset-modal');
    if (!modal)
        return;
    isModalOpen = false;
...
function populatePresetForm(preset) {
    ...
    if (preset) {
        ...
        if (instructionsField)
            instructionsField.value = preset.instructions;
        if (modelField)
...
async function handlePresetSave() {
    try {
        ...
        agentPresets.handleAgentPresetError(error, currentEditingPreset ? 'update' : 'create');
    }
    finally {
...
async function handlePresetDelete() {
    ...
    try {
        ...
        agentPresets.handleAgentPresetError(error, 'delete');
    }
    finally {
...
function getFormData() {
    ...
    return {
        ...
        model: modelField?.value || 'gpt-5.3',
 ...
 */
function showAgentPresetWarning(message) {
 ...
 */
function showAgentPresetError(message) {
...
function renderPresetList() {
    ...
    if (!listContainer || !loadingElement || !noPresetsElement)
        return;
    // Show loading state
 ...
 */
export function editAgentPreset(presetId) {
    const preset = allAgentPresets.find(p => p.id === presetId);
    if (preset) {
        openAgentPresetModal(preset);
    }
