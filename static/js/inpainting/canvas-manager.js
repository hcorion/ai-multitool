...
export class CanvasManager {
     ...
     */
    renderColorLayer(dirtyRect) {
        if (!this.state || !this.state.colorData)
            return;
        ...
        for (let y = 0; y < rect.height; y++) {
            for (let x = 0; x < rect.width; x++) {
                const imgX = rect.x + x;
                const imgY = rect.y + y;
                if (imgX < 0 || imgX >= this.state.imageWidth || imgY < 0 || imgY >= this.state.imageHeight)
                    continue;
     ...
     */
    updateColorLayerDisplay(dirtyRect) {
        if (!this.state)
            return;
    ...
    validateExport() {
        if (!this.state)
            throw new CanvasValidationError('No image loaded', 'NO_IMAGE');
     ...
     */
    performMaskOverlayUpdate(dirtyRect) {
        if (!this.state)
            return;
     ...
     */
    updateColorData(x, y, r, g, b) {
        if (!this.state)
            return false;
        if (x < 0 || x >= this.state.imageWidth || y < 0 || y >= this.state.imageHeight)
            return false;
        if (!this.state.colorData)
            this.initializeColorLayer();
     ...
     */
    calculateBrushDirtyRect(centerX, centerY, brushSize) {
        if (!this.state) {
