...
export class VibeSelectionModal {
    ...
    attachEventListeners() {
        ...
        // Strength sliders
        const encodingSlider = document.getElementById('encoding-strength-slider');
     ...
     */
    show(currentModel, onVibeSelected) {
        if (!this.modal)
            return;
     ...
     */
    showSelectionControls(selectedVibe, details) {
    ...
    updatePreviewImage(selectedVibe, details) {
        const previewImg = document.getElementById('vibe-preview-image');
        if (!previewImg)
            return;
    ...
    updateEncodingStrength() {
        const slider = document.getElementById('encoding-strength-slider');
        if (!slider || this.selectedVibes.length === 0)
            return;
    ...
    updateReferenceStrength() {
        const slider = document.getElementById('reference-strength-slider');
        if (!slider || this.selectedVibes.length === 0)
            return;
     ...
     */
    validateEncodingStrength(encodingStrength) {
     ...
     */
    validateReferenceStrengthRange(referenceStrength) {
        return referenceStrength >= 0.0 && referenceStrength <= 1.0;
    }
    /**
     * Validate model compatibility
     */
    validateModelCompatibility(vibeModel, currentModel) {
